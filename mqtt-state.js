...
const isInterestingDevice = function(deviceTopic) {
	...
	if (!deviceTopic.startsWith('/')) {
		return false
	}
	if (deviceTopic.length == 1) { 
		return false
	}
	if (deviceTopic.includes('/isy')) { 
		return false 
	}
	if (deviceTopic.includes('test')) {
		return false 
	}
	if (deviceTopic.endsWith('/set')) {
		return false
	}
	if (deviceTopic.startsWith('/homeseer/action/')) {
		return false
	}
	if (deviceTopic.startsWith('happy')) { 
		return false 
	}
	if (deviceTopic.startsWith('/deconz')) { 
		return false
	}
	if (deviceTopic.startsWith('/Deconz')) { 
		return false 
	}
	if (deviceTopic.startsWith('/vera')) { 
		return false 
	}
	if (deviceTopic.startsWith('/sonos')) {
		return false
	}
	if (deviceTopic.startsWith('/hubitat')) { 
		return false 
	}
	if (deviceTopic.startsWith('/openmqtt')) { 
		return false
	}
	if (deviceTopic.startsWith('/xiaomi')) { 
		return false 
	}
